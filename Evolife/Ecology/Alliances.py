...
class club(object):
	...
	def names(self):	return [T[0] for T in self.__members]

	def performances(self):	return [T[1] for T in self.__members]
		
	def present(self, MemberPerf):	return MemberPerf in self.__members
	...
	def minimal(self):
		...
		if self.size():	return min([T[1] for T in self.__members])
		return -1

	def maximal(self):
		" returns the maximal performance among members "		
		if self.size():	return max([T[1] for T in self.__members])
		return -1
	...
	def best(self):
		...
		if self.size():	return max(self.__members, key=lambda x: x[1])[0]
		return None
	...
	def exits(self, oldMember):
		...
		for (M,Perf) in self.__members[:]:  # safe to copy the list as it is changed within the loop
			if M == oldMember:
				...
				return True
		print('members: %s' % str([(str(F[0]),F[1]) for F in self.__members]))
...
class Friend:
	...
	def detach(self):
		...
		for F in self.friends.names():	self.quit_(F)
	...
	def forgetAll(self):
		""" The individual quits its friends	"""
		for F in self.friends.names():	self.end_friendship(F)
...
class Follower(Friend):
	...
	def __init__(self, MaxGurus, MaxFollowers=0):
		...
		if MaxFollowers:
			self.followers = Friend(MaxFollowers)	# to keep track of followers
