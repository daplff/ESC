...
class Swallow_Observer(Experiment_Observer):
	...
	def __init__(self):
		...
		self.curve('#indiv', 'blue', 'number of individuals perched')
		self.curve('timeslope', 'yellow', 'perching probability')
	...
	def get_info(self, Slot):
		if Slot == 'PlotOrders':
			return [(self.curve('#indiv'), (self.StepId, 100.0 * self.Perched/len(self.Positions))),	# number of individuals perched
					(self.curve('timeslope'), (self.StepId,
