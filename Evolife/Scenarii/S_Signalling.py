...
##                Partner.id, Partner.Phene_value('Signal'), PartnerOffer)
		...
		if not Indiv.get_friend(IndivOffer, Partner, PartnerOffer):
			...
			print('%s was accepted by %s with offer-: %d' % (Indiv.id, Partner.id, IndivOffer))
...
##        print PartnerFriends, 'becomes', Partner.friends()
       ...
       # # # Indiv.follow(self.SocialOffer(Indiv, 0), Partner, self.SocialOffer(Partner, 0))
       # # # Partner.follow(self.SocialOffer(Partner, 0), Indiv, self.SocialOffer(Indiv, 0))
...
##            Partner.follow(0, Indiv, IndivOffer)
